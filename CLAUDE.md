...
- Fonts all self-hosted via `next/font` — **no runtime request ever reaches Google** (GDPR / the German "Google Fonts" case law): Fraunces (serif headings) + Inter (sans body) via `next/font/google` (downloaded and served from our origin at build); Noto Sans JP/KR via `next/font/local` against woff2 files in `app/fonts/` (full japanese/korean subsets from Fontsource), loaded only on ja/ko pages via `html:lang` scoping in `app/globals.css`. See `lib/fonts.ts` (ADR-019)
...
- Lucide icons; no other UI framework (official brand glyphs — Bluesky, X, Instagram — are inline SVGs in `components/social-links.tsx`)
- Official social profiles live in `siteConfig.socialLinks` (`lib/site.ts`): Bluesky, X and Instagram — rendered by `components/social-links.tsx` in the footer on every page and listed as Organization `sameAs` in the site-wide JSON-LD. Only real, active profiles may be added (no dead social icons)
...
- Supabase database (project ref `hjpfycbamwwpemsrrsqy`, ADR-008): `profiles` plus the payment domain tables (ADR-009), schema managed via CLI migrations in `supabase/migrations/` (`supabase db push`). The legacy `early_access_signups` table is retired — the marketing early-access form was removed once real magic-link sign-in went live; the table remains (frozen, no reads/writes) but is no longer part of the product
...
- **Journey — social timeline (ADR-025, Phase 2 core slice; built, staged, not released)** — the `creator_updates` feed is evolved into `journey_posts` (rename preserves data/RLS): `lib/journey/` domain boundary with posts, public feed, likes/comments interactions, a pure `milestonesCrossed()` helper, and a service-role draft writer. New tables `journey_media` (multi-image), `journey_comments` (flat, signed-in only, creator-moderated), `journey_likes` (one per user); denormalised `like_count`/`comment_count` maintained by `SECURITY DEFINER` triggers with no client write grant. **Automatic milestone posts (goal 25/50/75/100%) are created as DRAFTS by the verified webhook path only** (`markGiftPaidVerified`, idempotent via a partial unique index) — never auto-published, never client-invented (ADR-011/016/022); `createGoal` seeds a "new goal" draft; draft copy localised at write time. UI: `components/ui/` `ProgressRing`/`MilestoneBadge`, redesigned profile header (ring + latest milestone), sticky in-page `ProfileTabs` (About/Journey/Goals/Support, keeps SSG), `PublicJourney` feed with client like/comment islands + media/video/milestone badges, dashboard `JourneyManager` (manual milestones, video, goal link) and `DashboardStats` (verified figures + honest "coming soon" tiles for recurring/profile-views). API under `/api/journey/**` (auth-gated, rate-limited); `journey` message namespace ×8 locales. Migration `20260725193000_journey.sql` awaits application to a sandbox. Deferred: profile analytics/traffic sources, visual timeline, reputation, recurring-support tables, in-app notifications, Journey-everywhere, per-post OG images
